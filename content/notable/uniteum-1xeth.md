...
[`Uniteum 1xETH`](https://etherscan.io/token/) is an ERC-20 that always trades 1:1 against ETH. One `Uniteum 1xETH` is worth one ETH, give or take a hundredth of a percent — and every token in circulation is backed, right now, by an equal amount of real ETH locked in a Uniswap V4 pool.
...
1. Open the [Notable prototype](/notable/) on Etherscan: [`{{< val "unispring.notable.address" >}}`](https://etherscan.io/address/{{< val "unispring.notable.address" >}}#writeContract).
2. Connect a wallet with enough ETH to cover gas. (No collateral capital is needed — the mint funds itself.)
3. Call [`issue(name)`](https://etherscan.io/address/{{< val "unispring.notable.address" >}}#writeContract#F{{< val "unispring" "notable" "write" "issue(name)" "f" >}}) with the name you want — `"Acme 1xETH"`, `"Bob 1xETH"`, whatever.
...
To preview the deterministic address before deploying, call [`issued(name)`](https://etherscan.io/address/{{< val "unispring.notable.address" >}}#readContract#F{{< val "unispring" "notable" "read" "issued(name)" "f" >}}) with the same name on the read tab.
...
For any other peg pair (`(USDC, "1xUSDC")`, `(WBTC, "1xWBTC")`, etc.), `issue(name)` is called on the corresponding clone instead. See [Uniteum 1xUSDC](/notable/uniteum-1xusdc/) for an example with a non-native original.
...
- [Uniteum 1xUSDC](/notable/uniteum-1xusdc/) — the same pattern applied to a stablecoin original

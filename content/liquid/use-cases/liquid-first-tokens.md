...
At equilibrium, 1 liquid = 1 solid. The [2x mint](/liquid/2x-mint) mechanism preserves this: every heat deposits solid and mints liquid in equal measure, and every cool burns liquid and returns solid. The math enforces the peg without oracles, governance, or trust.
...
1. **Deploy your backing token** with the logic your project needs. Use [Lepton](/lepton/) for a simple fixed-supply token, or write a custom contract with whatever minting, governance, or access control you require.
...
3. **Seed liquidity.** Call `heat` to deposit backing tokens and create pool depth. The [2x mint](/liquid/2x-mint) ensures that every deposit splits evenly between your wallet and the pool.

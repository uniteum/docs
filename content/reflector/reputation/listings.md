...
- **Verify the signature token's address directly** on the block explorer — the [par-tokens directory](/reflector/par-tokens/) links each one, and the [factory reference](/reflector/reference/#how-to-use-it) shows how addresses are deterministic in `(clone, name)`.

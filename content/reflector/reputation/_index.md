...
---
# Why a Reflector issue needs no reputation
...
| Signal | The trust question it proxies | Why it's moot for a Reflector issue |
|:--|:--|:--|
...
| [Audits](/reflector/reputation/audits/) | Does the code hide a rug? | The trust surface is geometric and tiny; every issue is byte-identical |
...
| [Age & track record](/reflector/reputation/age/) | Has it survived without rugging? | It can't rug at any age; a one-block-old issue has the same guarantee |
...
Stripping away reputation doesn't make a Reflector issue risk-free. It relocates *all* of the risk to a single place: **the backing token**. An issue inherits every risk of the original it mirrors — if the backing depegs, freezes, or fails, the issue follows it down. The peg only promises that the issue tracks the original; it makes no promise about the original itself.
...
2. **The backing is sound** — the only judgment call left, and it's a judgment about the *original*, not the issue.
...
- [Par tokens](/reflector/par-tokens/) — the directory of issues minted so far

...
**The signal:** How big and active is the community? Followers on X, members in Discord and Telegram, the energy of the chat, which influencers are talking about it. A loud, growing community reads as a token with a future; silence reads as a token already dead.

For an ordinary token, social proof is a proxy for **future demand** — and future demand is, for most tokens, the *entire* basis of value. A token with no cash flows is worth what the next person will pay, and the size and conviction of the community is your best read on whether that next person exists. Hype is, quite literally, the fundamentals. So people check the social signal because for a discretionary-value token it's not vanity; it's the asset.
...
This also makes social proof useless as a *manipulation tell*: with ordinary tokens, manufactured hype (bot followers, paid shills) is a warning sign worth checking for. A par token removes the incentive — there's nothing to pump, so faked community buys the faker nothing.
...
Don't gauge the chat. The thing social proof stands in for — "will this still be worth something later?" — is answered by the peg, not the people:

- **Redeemable at par regardless of sentiment** — see [Peg mechanics](/reflector/mechanics/).
- **The backing token** — whose own community and adoption *do* matter, because that's where discretionary value (if any) actually lives.

...
A [signature token](/reflector/) has no price consensus to break, so concentration carries no threat.
...
The same explorer page that misreads a par token's [FDV](/reflector/fdv/) also misreads its holder distribution: it will flag "1 holder owns 100%" early in a signature token's life as a concentration warning. It's describing the gift you just minted, not a risk. The metric is correct; the interpretation it invites is wrong.

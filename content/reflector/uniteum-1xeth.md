...
## Swap ETH for `Uniteum Ether`

Use any V4-aware DEX or aggregator — Uniswap's own UI, 1inch, Matcha, CoW Swap, custom routers — and pick `ETH ↔ Uniteum Ether`. There is no separate "redeem" function: the pool is the redemption path.

- `ETH → Uniteum Ether` to acquire
- `Uniteum Ether → ETH` to redeem
...
3. Call [`issue(name)`](https://{{< escan >}}/address/{{< val "unispring.reflector.address" >}}#writeContract#F{{< val "unispring" "reflector" "write" "issue(name, supply, variant)" "f" >}}) with the name you want — `"Acme 1xETH"`, `"Bob 1xETH"`, whatever.
...
To preview the deterministic address before deploying, call [`issued(name)`](https://{{< escan >}}/address/{{< val "unispring.reflector.address" >}}#readContract#F{{< val "unispring" "reflector" "read" "issued(name, supply, variant)" "f" >}}) with the same name on the read tab.
...
For any other peg pair (`(USDC, "1xUSDC")`, `(WBTC, "1xWBTC")`, etc.), `issue(name)` is called on the corresponding clone instead. See [Uniteum Dollar](/reflector/uniteum-1xusdc/) for an example with a non-native original.

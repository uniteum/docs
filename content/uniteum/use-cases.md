...
- [0xUSDC](/uniteum/reference/anchored-units/usdc/) — backed 1:1 by real USDC held in the contract
- `1/0xUSDC` — synthetic reciprocal (not backed)

**Price relationship:** `price(0xUSDC) × price(1/0xUSDC) = price(1)`

When USDC depegs:

| USDC Price | 0xUSDC Value | 1/0xUSDC Value | Effect |
|------------|-------------|---------------|--------|
| $1.00 | ~1 "1" | ~1 "1" | Equilibrium |
| $0.90 | ↓ 10% | ↑ 11% | Hedge gains |
| $0.50 | ↓ 50% | ↑ 100% | Strong protection |

**Hold both:** Your `1/0xUSDC` gains offset `0xUSDC` losses automatically.
...
**[0xWETH](/uniteum/reference/anchored-units/weth/) and `1/0xWETH`:**
- Long ETH? Hold [0xWETH](/uniteum/reference/anchored-units/weth/)
- Short ETH? Hold `1/0xWETH`
- Hedged exposure? Hold both

**[0xWBTC](/uniteum/reference/anchored-units/wbtc/) and `1/0xWBTC`:**
...
- ✅ No collateral for synthetic side (`1/0xUSDC` isn't backed)
...
**`0xWETH^2` (ETH squared):**

Price relationship: `price(0xWETH^2) = price(0xWETH)²`

| ETH Moves | 0xWETH Change | 0xWETH^2 Change |
|-----------|--------------|----------------|
| 2x | +100% | +300% (4x) |
| 1.5x | +50% | +125% (2.25x) |
| 0.5x | -50% | -75% (0.25x) |
...
**`0xWETH^3:2` (ETH to the 3/2 power):**
...
Price relationship: `price(0xWETH*0xWBTC) = price(0xWETH) × price(0xWBTC)`
...
**`0xWBTC*0xWETH/0xUSDC^2` — complex multi-asset derivative:**
- Long BTC+ETH
- Inverse squared USD exposure
- Custom risk profile

**`1/(0xWETH*0xWBTC)` — short the crypto market:**
- Inverse of basket
...
**Long `0xWETH/0xWBTC`, short `0xWETH*0xWBTC`:**
...
See [Anchored Units Reference](/uniteum/reference/anchored-units/) for complete list and usage examples.
...
- [Getting Started](/uniteum/getting-started/) — Try it yourself
- [Concepts](/uniteum/concepts/) — Understand the mechanism
- [Guides](/uniteum/guides/) — Practical tutorials
- [Examples](/uniteum/examples/) — Concrete transactions

---

**Remember:** This is {{< val "contracts.uniteum.name" >}} — experimental, unaudited, novel mechanism design. Proceed with curiosity and caution.

...
In Uniteum documentation, `0xUSDT` is a **readable shorthand** for an anchored unit backed by {{< token address=usdt.address text="Tether USD (USDT)" >}}.

The actual Uniteum symbol uses the full USDT contract address:
```
{{< val "tokens.USDT.address" >}}
```

## Backing Token

**USDT Contract:** {{< token address=usdt.address text=usdt.address >}}
...
IERC20 usdt = IERC20({{< val "tokens.USDT.address" >}});
IUnit usdtUnit = one.anchored(usdt);

// usdtUnit.symbol() returns: "{{< val "tokens.USDT.address" >}}"
```

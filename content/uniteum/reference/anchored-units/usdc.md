...
In Uniteum documentation, `0xUSDC` is a **readable shorthand** for an anchored unit backed by {{< token address=usdc.address text="USD Coin (USDC)" >}}.

The actual Uniteum symbol uses the full USDC contract address:
```
{{< val "tokens.USDC.address" >}}
```

## Backing Token

**USDC Contract:** {{< token address=usdc.address text=usdc.address >}}
...
IERC20 usdc = IERC20({{< val "tokens.USDC.address" >}});
IUnit usdcUnit = one.anchored(usdc);

// usdcUnit.symbol() returns: "{{< val "tokens.USDC.address" >}}"
```

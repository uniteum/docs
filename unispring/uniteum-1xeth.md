...
Crucially, the wallet that mints it is the on-chain deployer of that ERC-20 — which is the lever block explorers and token registries use to verify ownership when you submit token info. So `{{ mimic.name }}`'s icon, description, and project links on Etherscan are ours to set; `Acme 1xETH`'s would be Acme's. Their backing guarantees are identical; the brand on the token is not.

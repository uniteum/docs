...
There's a separate Notable family per backing token. To set up a new family, someone calls [`make(token, symbol)`]({% if n.address %}https://etherscan.io/address/{{ n.address }}#writeContract#F{{ n.write["make(original, symbol)"].f }}{% else %}#{% endif %}){:target="_blank"} on the Notable factory once. By convention the symbol is `1x` + the original — `1xUSDC`, `1xWBTC` — but it's whatever the maker types, so a family is only as trustworthy as whoever set it up.

After the family exists, anyone can call [`issue("Some Name")`]({% if n.address %}https://etherscan.io/address/{{ n.address }}#writeContract#F{{ n.write["issue(name)"].f }}{% else %}#{% endif %}){:target="_blank"} inside it to mint their own Notable. They all share the family symbol but each has its own name and its own pool.
